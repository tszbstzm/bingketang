...
import { EMAIL_ADDRESS_TEXT, ERROR_EMAIL, ERROR_NICKNAME, ERROR_PASSWORD, ERROR_PASSWORD_AGAIN, NICK_NAME_TEXT, PASSWORD_AGAIN_TEXT, PASSWORD_TEXT, REGISTER_NEED_EMAIL, REGISTER_NEED_NICKNAME, REGISTER_NEED_PASSWORD, REGISTER_TEXT } from "@/constant/text";
...
const RegisterPage = () => {
  ...
  return (
    <Form className={classnames(style.registerpage)}>
    ...
    <Form.Item className={classnames(style.formitem)} name="passwordagin" label={PASSWORD_AGAIN_TEXT}
      rules={[{ required: true, whitespace: true, message: REGISTER_NEED_PASSWORD }, 
        ({ getFieldValue }) => ({
          validator(_, value) {
    ...
    <Form.Item className={classnames(style.formitem)} >
      <Button className={classnames(style.submitbutton)} type="primary" htmlType="submit" onClick={handleRegister}>{REGISTER_TEXT}</Button>
    </Form.Item>
  </Form>
  );
};
